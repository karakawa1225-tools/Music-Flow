...
export function createApp() {
  ...
  api.get('/tracks/:id/stream', async (c) => {
    ...
    if (!track) return c.json({ error: 'Not found' }, 404)
    const filePath = resolve(AUDIO_DIR, track.path)
  ...
  api.post('/upload', async (c) => {
    ...
    const id = randomUUID()
    const storagePath = `${user.id}/${id}.mp3`
    const absDir = join(AUDIO_DIR, user.id)
    mkdirSync(absDir, { recursive: true })
    const absPath = join(AUDIO_DIR, storagePath)
